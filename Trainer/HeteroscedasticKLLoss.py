...
class HeteroscedasticKLLoss:
    """
    Heteroscedastic loss combining Gaussian NLL and KL divergence penalty.
    ...
    3. Feature-specific weighting for KL regularization
    """
    
    def __init__(self, empirical_var, kl_feature_weights, cont_idx, device):
        """
    ...
    def forward(self, mean, logvar, targets, kl_weight):
        """
        Compute heteroscedastic loss.

        heteroscedastic loss = NLL + kl_weight * KL
        ...
        for j, target in enumerate(targets):
            ...
            logvar_clean = torch.clamp(logvar[j, :L, :], min=-6.0, max=6.0)
        ...
        if valid_count > 0:
            ...
            total_loss = nll_loss_avg + kl_weight * kl_loss_avg
        else:
